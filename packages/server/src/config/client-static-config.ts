...
export function createClientStaticOptions(clientDist: string): FastifyStaticOptions {
  ...
  return {
    root: clientDist,
    prefix: "/",
    // Resolve files per request. With `wildcard: false` the plugin registers one
    // route per file that exists at boot, so a client rebuilt while the server
    // runs produces hashed bundles that fall through to the SPA fallback.
    wildcard: true,
    decorateReply: false,
