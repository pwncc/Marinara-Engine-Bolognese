...
/** Single-request account edit used by the in-core Noodle UI: identity fields plus optional
 *  profile, follow, and social patches. */
export const noodleAccountEditSchema = noodleAccountUpdateSchema
  .extend({
    profile: noodleAccountProfileSettingsSchema.partial().optional(),
    follow: z.object({ targetAccountId: z.string().min(1), followed: z.boolean() }).strict().optional(),
    social: noodleAccountSocialPatchSchema.partial().optional(),
  })
  .strict();
...
export type NoodleAccountEditInput = z.infer<typeof noodleAccountEditSchema>;
