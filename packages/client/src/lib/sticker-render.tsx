...
export function renderWithStickerBlocks(
  ...
  while ((match = re.exec(content)) !== null) {
    ...
    if (match.index > lastIndex) {
      parts.push(renderText(content.slice(lastIndex, match.index), `sc-t${segment}`));
    }
  ...
  if (lastIndex < content.length) parts.push(renderText(content.slice(lastIndex), `sc-t${segment}`));
  return <>{parts}</>;
}
