...
export function readAssistantPrefillFromExtra(extra: Record<string, unknown>): string | null {
  ...
  return typeof record.assistantPrefill === "string" && record.assistantPrefill.trim() ? record.assistantPrefill : null;
}
      ...
      >
        <div className="flex items-center justify-between border-b border-[var(--border)] px-4 py-3">
          <div className="flex items-center gap-2 text-sm font-semibold text-[var(--foreground)]">
            ...
            {localizeUi("ui.chat.modelthoughtsmodal.thoughtsSteering")}
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="space-y-4 overflow-y-auto px-4 py-3">
          {hasPrefill && (
            <section className="space-y-1.5">
              <h3 className="text-[0.75rem] font-semibold uppercase tracking-normal text-[var(--muted-foreground)]">
                {localizeUi("ui.chat.modelthoughtsmodal.assistantPrefillLastApiMessage")}
              </h3>
              <p className="text-[0.6875rem] leading-relaxed text-[var(--muted-foreground)]">
                {localizeUi("ui.chat.modelthoughtsmodal.partialAssistantTurnSentSoTheModelContinuesFrom")}
              </p>
          ...
          {hasThinking ? (
            <section className="space-y-1.5">
              <h3 className="text-[0.75rem] font-semibold uppercase tracking-normal text-[var(--muted-foreground)]">
                {localizeUi("ui.chat.modelthoughtsmodal.modelReasoningOutput")}
              </h3>
            ...
            !hasPrefill && (
              <p className="text-[0.8125rem] text-[var(--muted-foreground)]">
                {localizeUi("ui.chat.modelthoughtsmodal.noReasoningContentWasReturned")}
              </p>
            )
          )}
        </div>
