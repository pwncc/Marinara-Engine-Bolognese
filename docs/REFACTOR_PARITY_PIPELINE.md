...
- Static and webhook custom tools can be saved, selected, advertised to models, and executed in the refactor runtime. Legacy script tools remain unsupported and must be converted before use.
...
- Static custom tools return their configured string result to the next agent LLM turn.
- Webhook custom tools POST arguments to the configured webhook URL and return the webhook response to the next agent LLM turn.
- Legacy script custom tools are a deliberate refactor compatibility boundary: `executionType: "script"` and `scriptBody` are not part of the create/update contract, the editor marks existing script rows as unsupported, and native execution returns `custom_tool_script_unsupported` with a convert-to-webhook-or-static migration hint.
- Imports or profiles that contain legacy script tools should preserve enough data for user review, but those tools must be converted to static results or webhooks before they can be selected for execution.
